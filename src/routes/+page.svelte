...
<section
  class="relative h-[100svh] w-full bg-brandDark overflow-hidden flex flex-col"
>
  <div class="h-[70%] w-full relative overflow-hidden">
    {#if loaded}
      <div
        ...
        />
        <div
          class="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-brandDark to-transparent pointer-events-none"
        ></div>
      </div>
    {/if}
  </div>
  ...
  <div
    class="h-[30%] w-full relative z-20 bg-brandDark flex flex-col justify-start items-center text-center px-4 pt-2"
  >
    {#if loaded}
      <h1
        in:fly={{ y: 50, duration: 1000, delay: 200, easing: quintOut }}
        class="text-5xl md:text-[7vw] font-black uppercase italic tracking-tighter leading-none mb-6 text-white"
      >
        Horse<span class="text-transparent text-stroke-blue blue-glow ml-2"
          >Master</span
        >
      </h1>
      ...
      >
        <a
          href="/contact"
          class="px-10 py-4 bg-brandBlue text-white font-black rounded-full shadow-[0_0_30px_rgba(37,99,235,0.3)] uppercase tracking-widest text-[10px] hover:scale-105 active:scale-95 transition-transform flex items-center justify-center"
          aria-label="Započnimo suradnju"
        >
          Započnimo suradnju
        </a>
        <a
          href="/gallery"
          class="px-10 py-4 bg-white/5 backdrop-blur-md border border-white/10 text-white font-black rounded-full uppercase tracking-widest text-[10px] hover:bg-white hover:text-brandDark active:scale-95 transition-all flex items-center justify-center"
      ...
      <div class="mt-auto mb-4 overflow-hidden">
        <span
          in:fade={{ delay: 1000 }}
          class="text-brandBlue/50 text-[9px] font-mono uppercase tracking-[0.4em]"
  ...
  <div
    class="absolute inset-0 pointer-events-none opacity-[0.03] mix-blend-overlay bg-[url('https://grainy-gradients.vercel.app/noise.svg')] z-50"
  ></div>
</section>
...
>
  ...
  >
    <div class="reveal-init" use:reveal>
      ...
      <p
        class="text-brandBlue text-[9px] font-black uppercase tracking-[0.3em] mt-3"
      >
        Godina
      </p>
    </div>
...
<section class="py-32 bg-brandDark relative z-10">
  <div class="max-w-[1400px] mx-auto px-6">
    <div class="mb-20 text-center reveal-init" use:reveal>
      ...
      >
      <h2
        class="text-5xl md:text-8xl font-black uppercase tracking-tighter italic text-brandLight leading-none"
      >
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      {#each services as service, i}
        ...
        >
          <div
            class="absolute -right-10 -top-10 text-[10rem] opacity-[0.03] group-hover:opacity-[0.08] group-hover:rotate-12 transition-all duration-1000 grayscale select-none pointer-events-none"
          >
            {service.icon}
          </div>
          <div class="relative z-10 flex flex-col h-full min-h-[200px]">
            <span
              class="text-5xl mb-8 block transition-transform duration-500 group-hover:scale-110 drop-shadow-xl"
              >{service.icon}</span
            >
            <h3
            ...
            <p
              class="text-brandLight/50 text-sm leading-relaxed group-hover:text-brandLight transition-colors"
            >
              {service.desc}
            </p>
            ...
            <div class="mt-auto pt-8 flex items-center justify-between">
              <div
                class="h-[2px] w-12 bg-white/10 group-hover:w-16 group-hover:bg-brandBlue transition-all duration-500 ease-[quintOut]"
              ></div>
              <span
                class="text-[9px] font-black uppercase tracking-widest text-brandBlue opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-x-2 group-hover:translate-x-0"
                >Detaljnije &rarr;</span
              >
            </div>
          </div>
        </a>
  ...
  <section class="py-32 bg-brandDark border-t border-white/5 relative z-10">
    <div class="max-w-[1400px] mx-auto px-6">
      <div class="flex items-end justify-between mb-20">
        <h2
          class="text-4xl font-black uppercase tracking-tighter text-brandLight italic"
        >
          Aktualno
        </h2>
        <a
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        {#each data.news as post, i}
          <button
            ...
            aria-label={`Otvori vijest: ${post.title}`}
          >
            {#if post.image_url}
...
{#if selectedNews}
  <div
    class="fixed inset-0 z-[150] bg-brandDark/98 backdrop-blur-2xl flex items-center justify-center p-4 md:p-12"
    transition:fade={{ duration: 300 }}
    role="dialog"
  >
    <button
      class="absolute inset-0 cursor-zoom-out appearance-none bg-transparent border-none w-full h-full"
      onclick={closeNews}
      aria-label="Zatvori modal klikom u pozadinu"
    ></button>
    <div
      class="relative bg-brandDeep/40 w-full max-w-4xl h-full md:h-auto md:max-h-[90vh] overflow-y-auto rounded-[3rem] border border-white/5 shadow-2xl no-scrollbar"
    ...
    >
      ...
      >
        <button
          onclick={closeNews}
          class="w-12 h-12 bg-white/10 backdrop-blur-md rounded-full text-white flex items-center justify-center hover:bg-white hover:text-brandDark transition-all pointer-events-auto border-none"
          aria-label="Zatvori"
          ><svg
            class="w-6 h-6"
            fill="none"
      ...
      {#if selectedNews.image_url}
        <div class="h-[40vh] md:h-[50vh] w-full relative shrink-0">
          ...
          />
          <div
            class="absolute inset-0 bg-gradient-to-t from-brandDark to-transparent"
          ></div>
        </div>
      {/if}
      <div class="p-10 md:p-16 relative z-10">
        ...
        >
        <h2
          class="text-4xl md:text-6xl font-black text-white mb-10 leading-none uppercase italic"
        >
          {selectedNews.title}
        </h2>
